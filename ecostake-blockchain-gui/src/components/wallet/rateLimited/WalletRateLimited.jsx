...
import { AlertDialog, Card, Flex } from '@ecostake/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const ecostakeper_input = ip_parsed.limit;
    ...
    const ecostakeper_value = Number.parseInt(Number(ecostakeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (ecostake per interval):{' '}
                  {mojo_to_ecostake_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = ecostake_to_mojo(fee_input.value);

...
from ecostake.cmds.init_funcs import check_keys, ecostake_init
from ecostake.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from ecostake.daemon.keychain_server import KeychainServer, keychain_commands
from ecostake.daemon.windows_signal import kill
from ecostake.plotters.plotters import get_available_plotters
from ecostake.plotting.util import add_plot_directory
from ecostake.server.server import ssl_context_for_root, ssl_context_for_server
from ecostake.ssl.create_ssl import get_mozilla_ca_crt
from ecostake.util.ecostake_logging import initialize_logging
from ecostake.util.config import load_config
from ecostake.util.json_util import dict_to_json_str
from ecostake.util.keychain import (
    Keychain,
...
from ecostake.util.service_groups import validate_service
from ecostake.util.setproctitle import setproctitle
from ecostake.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Ecostake.")
    quit()
...
service_plotter = "ecostake plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "ecostake": "ecostake",
        "ecostake": "ecostake",
        "ecostake_wallet": "start_wallet",
        "ecostake_full_node": "start_full_node",
        "ecostake_harvester": "start_harvester",
        "ecostake_farmer": "start_farmer",
        "ecostake_introducer": "start_introducer",
        "ecostake_timelord": "start_timelord",
        "ecostake_timelord_launcher": "timelord_launcher",
        "ecostake_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["ecostake", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("ecostake daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered ECOSTAKE_ROOT
    os.environ["ECOSTAKE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with ECOSTAKE_ROOT: {os.environ['ECOSTAKE_ROOT']}")
    ...
    if service_command == "ecostake_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in ecostake_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    ecostake_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("ecostake_daemon")

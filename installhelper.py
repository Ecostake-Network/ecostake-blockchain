...
# Gets the version from the result of "ecostake version"
...
def update_version():
    with open(f"{os.path.dirname(__file__)}/ecostake-blockchain-gui/package.json") as f:
        data = json.load(f)

    version: str = "0.0"
    output = subprocess.run(["ecostake", "version"], capture_output=True)
    ...
    with open(f"{os.path.dirname(__file__)}/ecostake-blockchain-gui/package.json", "w") as w:

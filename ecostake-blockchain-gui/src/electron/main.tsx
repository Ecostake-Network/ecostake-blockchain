...
import ecostakeEnvironment from '../util/ecostakeEnvironment';
import ecostakeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!ecostakeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    ecostakeConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
     ...
     // console.log('startUrl', startUrl);
      ...
      if (!ecostakeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !ecostakeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (ecostakeConfig.manageDaemonLifetime()) {
        ecostakeEnvironment.startEcostakeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
		  {
            label: i18n._(/* i18n */ { id: 'Ecostake Blockchain Website' }),
            click: () => {
              openExternal(
                'https://ecostake.online',
              );
            },
          },
		  {
            ...
            click: () => {
              openExternal(
                'https://www.ecostake.online/faq',
              );
            },
          },
		  {
            label: i18n._(/* i18n */ { id: 'Ecostake Calculator' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Ecostake Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Ecostake-Network/ecostake-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Ecostake-Network/ecostake-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Ecostake-Network/ecostake-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Ecostake-Network/ecostake-blockchain/issues',
              );
            },
          },
		  {
		  ...
		  {
            label: i18n._(/* i18n */ { id: 'Join Ecostake on Discord' }),
            click: () => {
              openExternal('https://discord.gg/wVAd75mJYR');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow Ecostake on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/Ecostake-NetworkNet');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Ecostake Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Ecostake' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Ecostake Blockchain' }),
            click: () => {
              openAbout();
            },
          },

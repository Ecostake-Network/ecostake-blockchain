...
from ecostake.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from ecostake.plotters.chiapos import get_chiapos_install_info, plot_ecostake
from ecostake.plotters.madmax import get_madmax_install_info, plot_madmax
from ecostake.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in ECOSTAKE_ROOT.
    ECOSTAKE_ROOT_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within ECOSTAKE_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, ecostake_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_ecostake(args, ECOSTAKE_ROOT_path)
    if args.plotter == "madmax":
        plot_madmax(args, ECOSTAKE_ROOT_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, ECOSTAKE_ROOT_path, root_path)

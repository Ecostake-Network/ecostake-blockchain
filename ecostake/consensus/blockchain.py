...
from ecostake.consensus.block_body_validation import validate_block_body
from ecostake.consensus.block_header_validation import validate_finished_header_block, validate_unfinished_header_block
...
from ecostake.consensus.multiprocess_validation import (
    PreValidationResult,
    _run_generator,
    pre_validate_blocks_multiprocessing,
)
from ecostake.consensus.pos_quality import UI_ACTUAL_SPACE_CONSTANT_FACTOR
...
from ecostake.full_node.mempool_check_conditions import get_name_puzzle_conditions
...
from ecostake.types.blockchain_format.vdf import VDFInfo
...
from ecostake.types.generator_types import BlockGenerator, GeneratorArg
from ecostake.types.header_block import HeaderBlock
from ecostake.types.unfinished_block import UnfinishedBlock
from ecostake.types.unfinished_header_block import UnfinishedHeaderBlock
from ecostake.types.weight_proof import SubEpochChallengeSegment
from ecostake.util.errors import ConsensusError, Err
from ecostake.util.generator_tools import get_block_header, tx_removals_and_additions
from ecostake.util.ints import uint16, uint32, uint64, uint128
from ecostake.util.streamable import recurse_jsonify
    ...
    async def get_farmer_difficulty_coeff(
        ...
        if minimal_staking == 0 or staking < minimal_staking:
            ...
            if blocks == 0:
                ...
                # multiple 1000 to keep same scale as eco
        ...
        if (coeff < Decimal(1)):
            ...
            coeff = round(Decimal(.5) * round(coeff/Decimal(.5)), 1)

        log.debug(
            f"Diff: {coeff} Found Blks: {blocks} Tot Space: {network_space} Stake: {(round(staking / 1000000000000,2))} "
            f"Your Space: {space} Peak Height: {peak_height} method: {(sys._getframe().f_back.f_code.co_name)}"
         )

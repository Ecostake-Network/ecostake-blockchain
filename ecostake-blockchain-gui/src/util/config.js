...
global.daemon_rpc_ws = `wss://${self_hostname}:42040`;
...
function loadConfig(version) {
  try {
    ...
    // 1) ECOSTAKE_ROOT environment variable
    // 2) version passed in and determined by the `ecostake version` call

    // check if ECOSTAKE_ROOT is set. it overrides everything else
    console.log("version", version)
    const config_root_dir =
      'ECOSTAKE_ROOT' in process.env
        ? process.env.ECOSTAKE_ROOT
        : path.join(os.homedir(), '.ecostake', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 42040); // jshint ignore:line

...
async def summary(
    ...
    if amounts is not None:
        print(f"Total ecostake farmed: {amounts['farmed_amount'] / units['ecostake']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['ecostake']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['ecostake']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'ecostake start wallet' and 'ecostake wallet show'"
            )

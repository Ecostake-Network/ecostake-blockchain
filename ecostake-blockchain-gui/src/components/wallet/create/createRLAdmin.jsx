...
import { AlertDialog } from '@ecostake/core';
import {
  Typography,
  Button,
  Box,
...
import { ecostake_to_mojo } from '../../../util/ecostake';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      ecostakeper_input.value === '' ||
      Number(ecostakeper_input.value) === 0 ||
      !Number(ecostakeper_input.value) ||
      isNaN(Number(ecostakeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const ecostakeper = ecostake_to_mojo(ecostakeper_input.value);
    const ecostakeper_value = Number.parseInt(Number(ecostakeper));
    const userpubkey = userpubkey_input.value;
    const amount = ecostake_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = ecostake_to_mojo(fee_input.value);

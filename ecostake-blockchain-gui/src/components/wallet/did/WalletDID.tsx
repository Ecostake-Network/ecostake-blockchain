...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@ecostake/core';
import {
...
import { mojo_to_ecostake_string } from '../../../util/ecostake';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_ecostake_string(props.balance)} TECOSTAKE
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'ecostake_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'ecostake_addr://') {

...
import { mojo_to_ecostake } from '../../util/ecostake';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_ecostake(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_ecostake(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const ecostakeFees = blockRecord.fees
    ? mojo_to_ecostake(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: ecostakeFees ? `${ecostakeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Ecostake blockchain
            </Trans>
          </Back>
        }
        action={

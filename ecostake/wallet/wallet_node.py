...
from ecostake.daemon.keychain_proxy import (
    KeychainProxy,
...
from ecostake.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from ecostake.protocols import farmer_protocol, wallet_protocol
from ecostake.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from ecostake.util.errors import Err, ValidationError
from ecostake.util.ints import uint32, uint64, uint128
from ecostake.util.keychain import Keychain
from ecostake.util.lru_cache import LRUCache
from ecostake.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from ecostake.util.path import mkdir, path_from_root
from ecostake.util.profiler import profile_task
from ecostake.wallet.block_record import HeaderBlockRecord
from ecostake.wallet.derivation_record import DerivationRecord
from ecostake.wallet.settings.settings_objects import BackupInitialized
from ecostake.wallet.transaction_record import TransactionRecord
from ecostake.wallet.util.backup_utils import open_backup_file
from ecostake.wallet.util.wallet_types import WalletType
from ecostake.wallet.wallet_action import WalletAction
from ecostake.wallet.wallet_blockchain import ReceiveBlockResult
from ecostake.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[EcostakeServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'ecostake keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSEcostakeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSEcostakeConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSEcostakeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSEcostakeConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSEcostakeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSEcostakeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSEcostakeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSEcostakeConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:

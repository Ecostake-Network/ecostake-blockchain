...
electron-packager . Ecostake --asar.unpack="**/daemon/**" --platform=darwin \
--icon=src/assets/img/Ecostake.icns --overwrite --app-bundle-id=net.ecostake.blockchain \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Ecostake-darwin-arm64/Ecostake.app --platform=darwin \
...
mv Ecostake-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Ecostake-$ECOSTAKE_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Ecostake-darwin-arm64/Ecostake.app Ecostake-$ECOSTAKE_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.ecostake.blockchain \

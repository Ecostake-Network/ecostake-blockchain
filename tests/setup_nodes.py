...
from ecostake.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from ecostake.full_node.full_node_api import FullNodeAPI
from ecostake.server.start_farmer import service_kwargs_for_farmer
from ecostake.server.start_full_node import service_kwargs_for_full_node
from ecostake.server.start_harvester import service_kwargs_for_harvester
from ecostake.server.start_introducer import service_kwargs_for_introducer
from ecostake.server.start_service import Service
from ecostake.server.start_timelord import service_kwargs_for_timelord
from ecostake.server.start_wallet import service_kwargs_for_wallet
from ecostake.simulator.start_simulator import service_kwargs_for_full_node_simulator
from ecostake.timelord.timelord_launcher import kill_processes, spawn_process
...
from ecostake.util.ints import uint16, uint32
from ecostake.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["eco_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "ecostake")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["eco_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "ecostake")

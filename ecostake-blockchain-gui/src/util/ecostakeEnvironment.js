...
const PY_FOLDER = '../ecostake/daemon';
...
const getEcostakeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('ecostake');
  console.log("exePath", exePath)
  // first see if we can get a ecostake exe in a standard location relative to where we are
  try {
    ...
    console.log("e1", e1)
    // that didn't work, let's try as if we're in the venv or ecostake is on the path
    try {
      version = child_process

...
from ecostake.daemon.server import service_launch_lock_path, singleton
from ecostake.server.ssl_context import ecostake_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"ecostake_{service_name}"
        ...
        ecostake_ca_crt, ecostake_ca_key = ecostake_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = EcostakeServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (ecostake_ca_crt, ecostake_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for EcostakeServer to be closed")
